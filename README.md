...
| **Looking Glass**            | Web app that allows users to create watchlists and film recommendations based on what their friends have picked for them.| Designed the fetch functions to collect the data from the API. Used that data to create modal components for each film choice. These contained all the information pertaining to the film with the option to add to a user’s watchlist. |Ruby-on-Rails, React.js, Rspec |
| **AcebookCLOBS** | A social media clone that allows signed up users to write posts and view a news feed of existing posts with an ability to like and unlike as they wish. | Used effective techniques to learn React.js in a short time-frame. This allowed me to refractor our Rails code to give the user's homescreen complete single-page functionality. |Ruby-on-Rails, React.js, Rspec |
| **MakersBnB** | This website allows users to list their properties for rent, as well as view properties to rent themselves based on available dates. | Designed and implemented a dates reservation system that cross-checks a user's requested dates for properties in that range. This enabled owners to never have double booked properties.|Ruby, Rspec |
